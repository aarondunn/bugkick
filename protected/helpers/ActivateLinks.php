    ...
    public static function perform( $text ) {
    	...
    	foreach ( $textarr as $piece ) {
    		...
    		if ( 10000 < strlen( $piece ) ) {
    			...
    			$ret = preg_replace_callback( $url_clickable, array('ActivateLinks', '_make_url_clickable_cb'), $ret );

    			$ret = preg_replace_callback( '#([\s>])((www|ftp)\.[\w\\x80-\\xff\#$%&~/.\-;:=,?@\[\]+]+)#is', array('ActivateLinks', '_make_web_ftp_clickable_cb'), $ret );
    			$ret = preg_replace_callback( '#([\s>])([.0-9a-z_+-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,})#i', array('ActivateLinks', '_make_email_clickable_cb'), $ret );
...
/**
   ...
   public static function wp_kses_normalize_entities($string) {
   	...
   	$string = preg_replace_callback('/&amp;([A-Za-z]{2,8});/', array('ActivateLinks', 'wp_kses_named_entities'), $string);
   	$string = preg_replace_callback('/&amp;#(0*[0-9]{1,7});/', array('ActivateLinks', 'wp_kses_normalize_entities2'), $string);
   	$string = preg_replace_callback('/&amp;#[Xx](0*[0-9A-Fa-f]{1,6});/', array('ActivateLinks', 'wp_kses_normalize_entities3'), $string);
   ...
   public static function valid_unicode($i) {
    ...
    public static function wp_kses_decode_entities($string) {
    	$string = preg_replace_callback('/&#([0-9]+);/', array('ActivateLinks', '_wp_kses_decode_entities_chr'), $string);
    	$string = preg_replace_callback('/&#[Xx]([0-9A-Fa-f]+);/', array('ActivateLinks', '_wp_kses_decode_entities_chr_hexdec'), $string);
